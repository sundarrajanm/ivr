...
public class Routes extends AllDirectives {
    //#user-routes-class
    final private ActorRef ivrRegistryActor;
    final private LoggingAdapter log;


    public Routes(ActorSystem system, ActorRef ivrRegistryActor) {
        this.ivrRegistryActor = ivrRegistryActor;
    ...
    private Route getUser(String name) {
      return get(() -> {
          //#retrieve-user-info
          CompletionStage<Optional<UserRegistryActor.User>> maybeUser = PatternsCS
            .ask(ivrRegistryActor, new UserRegistryMessages.GetUser(name), timeout)
            .thenApply(obj ->(Optional<UserRegistryActor.User>) obj);
    ...
    private Route deleteUser(String name) {
      return
          //#users-delete-logic
          delete(() -> {
            CompletionStage<UserRegistryMessages.ActionPerformed> userDeleted = PatternsCS
              .ask(ivrRegistryActor, new UserRegistryMessages.DeleteUser(name), timeout)
              .thenApply(obj ->(UserRegistryMessages.ActionPerformed)obj);
            ...
            route(
                get(() -> {
                    CompletionStage<UserRegistryActor.Users> futureUsers = PatternsCS
                        .ask(ivrRegistryActor, new UserRegistryMessages.GetUsers(), timeout)
                ...
                post(() ->
                    entity(
                        ...
                        user -> {
                            CompletionStage<UserRegistryMessages.ActionPerformed> userCreated = PatternsCS
                                .ask(ivrRegistryActor, new UserRegistryMessages.CreateUser(user), timeout)
                                .thenApply(obj ->(UserRegistryMessages.ActionPerformed) obj);
